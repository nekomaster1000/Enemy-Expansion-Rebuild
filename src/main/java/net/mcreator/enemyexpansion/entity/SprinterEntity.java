...
import net.mcreator.enemyexpansion.procedures.SprinterHurtProcedure;
import net.mcreator.enemyexpansion.init.EnemyexpansionModEntities;

public class SprinterEntity extends Zombie implements IAnimatable {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(SprinterEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(SprinterEntity.class, EntityDataSerializers.STRING);
	...
	public SprinterEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(EnemyexpansionModEntities.SPRINTER.get(), world);
	}

	public SprinterEntity(EntityType<SprinterEntity> type, Level world) {
		super(type, world);
		xpReward = 5;
		setNoAi(false);
	}
	...
	public boolean hurt(DamageSource source, float amount) {
		SprinterHurtProcedure.execute(this.level, this, source.getEntity());
	...
	public static void init() {
		SpawnPlacements.register(EnemyexpansionModEntities.SPRINTER.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(SprinterEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
